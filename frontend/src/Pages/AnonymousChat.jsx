...
const AnonymousChat = () => {
  ...
  if (!userSessionIdRef.current) {
    userSessionIdRef.current = `user_${Date.now()}_${Math.floor(
      Math.random() * 10000
    )}`;
  ...
  const startMatchmaking = useCallback(async () => {
    ...
    if (isSearching || chatActive) {
      console.log(
        "Already searching or chat active, skipping new matchmaking attempt."
      );
      return;
    }
    ...
    try {
      console.log(
        `Attempting initial match for user: ${
          userSessionIdRef.current
        } with interests: ${interests.join(", ")}`
      );
      ...
      if (res.data.status === "match_found" && res.data.match_id) {
        ...
        if (pollingIntervalRef.current)
          clearInterval(pollingIntervalRef.current); // Clear interval if match found
        return;
        ...
        pollingIntervalRef.current = setInterval(async () => {
          try {
            console.log(
              `Polling for match for user: ${userSessionIdRef.current}`
            );
            const checkRes = await axios.post(
              "http://127.0.0.1:5000/process_match",
              {
                interests: interests,
                user_id: userSessionIdRef.current,
              }
            ...
            if (
              checkRes.data.status === "match_found" &&
              checkRes.data.match_id
            ) {
        ...
        setError(
          res.data.message || "An unexpected error occurred during matchmaking."
        );
    ...
    const unsubscribe = listenToAnonymousMessages(matchId, (messageList) => {
      ...
      setMessages(
        messageList.map((msg) => {
          ...
          const senderType =
            msg.anonymousUserId === userSessionIdRef.current
              ? "me"
              : "stranger";
          return {
            id: msg.id,
  ...
  const handleEndChat = async () => {
    ...
    try {
      console.log(
        `Ending match ${matchId} for user ${userSessionIdRef.current}`
      );
      ...
      setConnectionStatus(
        'Chat ended. Click "Start New Chat" to find another stranger.'
      );
    } catch (err) {
  ...
  useEffect(() => {
    return () => {
      ...
      if (matchId && userSessionIdRef.current) {
        ...
        axios
          .post("http://127.0.0.1:5000/end_match", {
            match_id: matchId,
            user_id: userSessionIdRef.current,
          })
    ...
    >
      ...
      >
        ...
        >
          <div
            style={{
              fontSize: "14px",
              fontWeight: "500",
              color: chatActive ? "#059669" : "#f59e0b",
            }}
          >
            {connectionStatus}
          </div>
        ...
        >
          ...
          >
            {chatActive
              ? `You are chatting with a stranger (${myAnonymousDisplayName}). Happy chatting!`
              : connectionStatus}
          </div>
          {chatActive && (
            <button
          ...
          {!chatActive &&
            !isSearching && ( // Show Start New Chat only if not active and not searching
              <button
                onClick={handleStartNewChat}
                style={{
            ...
            messages.map((msg) => (
              ...
              >
                ...
                <strong>
                  {msg.sender === "me"
                    ? "You"
                    : msg.anonymousDisplayName || "Stranger"}
                </strong>
                : {msg.text}
              </div>
            ))
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          <input
            ...
            placeholder={
              chatActive ? "Type your message..." : "Waiting for match..."
            }
